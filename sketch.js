...
resetButton.position(objectX-190, buttonStartY+30);
...
goButton.position(objectX-290, objectStartY + 10);
...
function updateValues() {
  ...
  // Show launch button after angle adjustment for rocket/iss
  if ((selectedObject === "rocket" || selectedObject === "iss") &&
...
function updateVelocitySlider() {
  velocitySlider?.hide();  // hide if exists

  if (selectedObject === "car") {
    velocityValue = 30;
    angleSlider.elt.min = 0;
    angleSlider.elt.max = 90;
    angleSlider.elt.step = 15;
  } else if (selectedObject === "plane") {
    velocityValue = 300;
    angleSlider.elt.min = 0;
    angleSlider.elt.max = 20;
    angleSlider.elt.step = 5;
  } else if (selectedObject === "rocket") {
    velocityValue = 150;
    angleSlider.elt.min = 45;
    angleSlider.elt.max = 90;
    angleSlider.elt.step = 15;
  ...
  // Apply immediately to slider value (if you show it later)
  angleSlider.value(angleValue);
}
...
function handleLauncherSequence() {
  if (launcherState === "rising") {
    ...
    if (launcherY <= launcherTargetY) {
      ...
      // ✅ Listen for angle adjustment after launcher ready
      previousAngle = angleValue;
      previousVelocity = velocityValue;
    }
  }
...
function handleRocketAndISSPhysics() {
  ...
  if (selectedObject === "rocket") {
    // Enter moon's orbit if close enough
    if (!inMoonGravity && distToMoon < moonGravityRadius) {
      inMoonGravity = true;
    ...
    if (inMoonGravity && !landingPhase) {
      // Circular orbit
      orbitAngle += 0.025;  // Controls speed of orbit
      orbitCounter += 0.025;

      // Start spiraling inward after 1.5 full orbits
      if (orbitCounter > TWO_PI * 1.5) {
        orbitRadius -= 0.3;  // Slow descent
      }

      // Set new position around moon
      ball.x = moonX + orbitRadius * cos(orbitAngle);
      ball.y = moonY + orbitRadius * sin(orbitAngle);

      // Begin landing if close enough
      if (orbitRadius <= 50) {
        landingPhase = true;
      }
    } 
    
    else if (inMoonGravity && landingPhase) {
      // Landing motion — continue decreasing orbit
      orbitRadius -= 0.2;
      if (orbitRadius > 35) {
        ball.x = moonX + orbitRadius * cos(orbitAngle);
        ball.y = moonY + orbitRadius * sin(orbitAngle);
        ...
        ball.vx = 0;
        ball.vy = 0;
        ball.x = moonX + 35 * cos(orbitAngle);
        ball.y = moonY + 35 * sin(orbitAngle);
      }
    }

    // 🚀 If not in orbit yet — normal projectile motion
    if (!inMoonGravity && !landingPhase) {
      ball.x += ball.vx * 0.1;
      ball.vy += ball.ay * 0.1;
      ball.y += ball.vy * 0.1;
  ...
  if (selectedObject === "iss") {
    if (ball.y > LAYER_LIMITS.thermosphere) {
      ...
      ball.vy = 0;
      ball.ay = 0;
      ball.y = LAYER_LIMITS.thermosphere;
      ball.vx = 5;
      ball.x += ball.vx;
